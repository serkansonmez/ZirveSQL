...
SELECT CAST(ROW_NUMBER() OVER(ORDER BY TARIH ASC,SAAT asc) AS INT)  AS SATIRNO,
CRK AS CARIKODU, 
STA AS CARIADI, 
CARIID AS CLIENTREF,
ISLEMAC AS ACIKLAMA,
...
CONVERT(NUMERIC(20,2),ISNULL(TUTARB,0)) as BORC,
CONVERT(NUMERIC(20,2),ISNULL(TUTARA,0)) AS ALACAK  FROM 
( 
SELECT CARIGEN.CRK,STA,CARIGEN.REF AS CARIID ,CARIHAR.*,VADEGUNU=DATEDIFF(day, TARIH, VADETARIHI) ,TOPLAMLAR.*,
...
(
    ...
    /*
		 UNION ALL
		  SELECT ODEMEPLAN.REF,TARIH,TARIH,'C',175,'Ödeme Planý',SUBE,ISLEMTIPI,
		  EVRAKNO,ACIKLAMA,0,ODEMETL,0,ODEMED,UGDK,
		  DOVIZC,REF,175,
        MASRAFMERKEZI,'','','T',
        TARIH,KULLANICI,'','',
        CARIP_ID,P_ID,'',350,CARIREF,0,0,CARISUBE
          FROM ODEMEPLAN WHERE 1=1
          AND CARIP_ID IN (SELECT P_ID FROM CARIGEN WHERE ((isNull(Detay,'')<>'H') AND (isNull(GORUNSUN,'')<>'HAYIR')) and (CRK='120.3170000900'))
          AND TARIH BETWEEN @TARIH1 AND @TARIH2
     */
...
AND (TARIH>='1.1.2022')and(TARIH<='31.12.2022')
 ORDER BY TARIH,SAAT,ONCELIK,ISLEMTURU,BORA,EVRAKNO
